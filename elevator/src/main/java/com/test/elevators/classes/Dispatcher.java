...
import com.test.elevators.states.GoingDownState;
import com.test.elevators.states.GoingUpState;
import com.test.elevators.states.RestingState;
import com.test.elevators.states.StopState;
...
public class Dispatcher {
	...
	public Elevator getClosestElevator(final int floor) {
		...
		for (Map.Entry<String, Integer> elevator : priority.entrySet()) {
			if (elevator.getValue() <= minimum
					&& elevators.get(elevator.getKey()).getElevatorState() instanceof GoingDownState
					&& !(elevators.get(elevator.getKey()).getElevatorState() instanceof StopState)) {
				minimum = elevator.getValue();
				idMinElevator = elevator.getKey();
				downExist = true;
			}
			if (elevator.getValue() <= minimum
					&& elevators.get(elevator.getKey()).getElevatorState() instanceof RestingState
					&& !(elevators.get(elevator.getKey()).getElevatorState() instanceof StopState)) {
				minimum = elevator.getValue();
			...
			if (elevator.getValue() <= minimum
					&& elevators.get(elevator.getKey()).getElevatorState() instanceof GoingUpState
					&& !(elevators.get(elevator.getKey()).getElevatorState() instanceof StopState)) {
				minimum = elevator.getValue();
